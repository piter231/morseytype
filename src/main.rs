...
fn latin_to_morse_mapping() -> HashMap<char, &'static str> {
    morse_mapping().iter().map(|(k, v)| (*v, *k)).collect()
}
...
fn main() -> io::Result<()> {
    ...
    let words = words.into_iter().take(word_count).collect::<Vec<_>>();
        ...
        match (space_pressed, prev_space_pressed) {
            ...
            (false, true) => {
                if let Some(start) = space_press_time {
                    ...
                    execute!(stdout, MoveTo(12, current_morse_line))?;
                    execute!(stdout, Clear(ClearType::UntilNewLine))?;
                    print!("{}", morse_output);

                    decoded_text = decode_morse(&morse_output);
                    execute!(stdout, MoveTo(9, decoded_line))?;
                    execute!(stdout, Clear(ClearType::UntilNewLine))?;
                    print!("{}", decoded_text);
    ...
    let (total_seconds, wpm) = if let Some(start) = start_time {
